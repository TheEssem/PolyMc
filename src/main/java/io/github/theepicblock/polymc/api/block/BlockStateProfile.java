...
public class BlockStateProfile {
    ...
    private static final Block[] CROP_BLOCKS = {Blocks.POTATOES, Blocks.CARROTS};
    private static final Block[] PRESSURE_PLATE_BLOCKS = {Blocks.HEAVY_WEIGHTED_PRESSURE_PLATE, Blocks.LIGHT_WEIGHTED_PRESSURE_PLATE};
    ...
    private static final Predicate<BlockState> CROP_FILTER = (blockState) -> {
        int age = blockState.get(CropBlock.AGE);
        return age != 0 && age != 2 && age != 4 && age != 7;
    };
    private static final Predicate<BlockState> PRESSURE_PLATE_FILTER = (blockState) -> {
    ...
    private static final BiConsumer<Block,PolyRegistry> DOUBLESLAB_ON_FIRST_REGISTER = (block, polyRegistry) -> {
        ...
        /*} else if (block == Blocks.POTATOES || block == Blocks.CARROTS) {
            ...
            polyRegistry.registerBlockPoly(block, new BlockPoly() {
                @Override
                public BlockState getClientBlock(BlockState input) {
                    return input.get(Properties.POWER) == 0 ? input.with(Properties.POWER, 0) : input.with(Properties.POWER, 1);
                }
                @Override public void addToResourcePack(Block block, ResourcePackMaker pack) {}
            });*/
        } else {
    ...
    private static final BiConsumer<Block,PolyRegistry> CROP_ON_FIRST_REGISTER = (block, polyRegistry) -> polyRegistry.registerBlockPoly(block, (input) -> {
        int age = input.get(CropBlock.AGE);
        if (age == 7) {
            return input.with(CropBlock.AGE, 7);
        } else if (age >= 4) {
    ...
    private static final BiConsumer<Block,PolyRegistry> PRESSURE_PLATE_ON_FIRST_REGISTER = (block, polyRegistry) -> polyRegistry.registerBlockPoly(block, (input) -> input.get(Properties.POWER) == 0 ? input.with(Properties.POWER, 0) : input.with(Properties.POWER, 1));
    ...
    // SUB PROFILES
    public static final BlockStateProfile CROP_SUB_PROFILE = new BlockStateProfile("crop", CROP_BLOCKS, CROP_FILTER, CROP_ON_FIRST_REGISTER);
    public static final BlockStateProfile PRESSURE_PLATE_SUB_PROFILE = new BlockStateProfile("pressure plate", PRESSURE_PLATE_BLOCKS, PRESSURE_PLATE_FILTER, PRESSURE_PLATE_ON_FIRST_REGISTER);
    ...
    public static final BlockStateProfile NO_COLLISION_PROFILE = combine("blocks without collisions", KELP_SUB_PROFILE, SAPLING_SUB_PROFILE, /*CAVE_VINES_SUB_PROFILE,*/ TRIPWIRE_SUB_PROFILE, SMALL_DRIPLEAF_SUB_PROFILE, OPEN_FENCE_GATE_PROFILE, CROP_SUB_PROFILE, PRESSURE_PLATE_SUB_PROFILE);
